        ...
        if (cmd.getName().equalsIgnoreCase("setspectator")) {
            if (sender instanceof Player p) {
                ...
                if (p.hasPermission("bw.setlobby")) {
                    try {
                        Factory.createConfiguration(p.getLocation(), "Spawn.Spectator", file, cfg);
                        YamlConfiguration cfg1 = Messages.cfg;
                        String msg1 = ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(cfg1.getString("Setspectator")));
